...
import 'package:mud_speak/model/Code.dart';
import 'package:mud_speak/model/ResultData.dart';
...
class HttpUtil {
  ...
  HttpUtil() {
    _baseUrl = GlobalConfig.url;
  }

//  void getRequest(String url) async {
//    Dio dio = new Dio();
//    var response = await dio.get(url, data: {"id": 12, "name": "chen"});
//  }
//
//  void postRequest(String url) async {
//    var dio = new Dio();
      ...
      if (option.contentType != null &&
        ...
        if (response.statusCode == 201 && responseJson["token"] != null) {
          ...
          await SpUtil.putString(
              GlobalConfig.TOKEN_KEY, optionParams["authorizationCode"]);
        }
      }
      if (response.statusCode == 200 || response.statusCode == 201) {
        return ResultData(
            response.data, true, Code.SUCCESS, headers: response.headers);
      }
    } catch (e) {
      print(e.toString() + url);
      return ResultData(
          response.data, false, response.statusCode, headers: response.headers);
    }
    return new ResultData(
        Code.errorHandleFunction(response.statusCode, "", noTip), false,
        response.statusCode);
  }
...
//    SpUtil.remove(GlobalConfig.TOKEN_KEY);
//  }
